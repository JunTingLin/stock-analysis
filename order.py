...
logging.info(f"1. 當前帳戶目前現金為: NT${acc.get_cash()}")
send_msg.append(f"1. 當前帳戶目前現金為: NT${acc.get_cash()}")

acc_position = acc.get_position()
logging.info(f"2. 當前帳戶目前持倉為: {acc_position.position}")
send_msg.append(f"2. 當前帳戶目前持倉為: ")
for position in acc_position.position:
    stock_id = position['stock_id']
    quantity = position['quantity']
    send_msg += f"{stock_id} : {quantity} 張\n"  # normalize() 移除小數點後多餘的零
...
if not acc_position.position:
    ...
    logging.info(f"今天是{today.date()}，帳戶股票部位都是空的狀況，初始化持倉")
    send_msg.append(f"今天是{today.date()}，帳戶股票部位都是空的狀況，初始化持倉")
    logging.info(f"將於下一個交易調整持倉為: {position_today.position}")
    send_msg.append(f"將於下一個交易調整持倉為: ")
    for position in position_today.position:
        stock_id = position['stock_id']
        quantity = position['quantity']
        send_msg.append(f"{stock_id} : {quantity} 張\n")
...
if not month_trading_days.empty:
    ...
    if today == first_trading_day:
        logging.info(f"今天是{today.date()}，為該月實際的第一個交易日，將執行換股調整")
        send_msg.append(f"今天是{today.date()}，為該月實際的第一個交易日，將執行換股調整")
        logging.info(f"將於下一個交易調整持倉為{position_today.position}")
        send_msg.append(f"將於下一個交易調整持倉為: ")
        for position in position_today.position:
            stock_id = position['stock_id']
        ...
        if remove_ids:
            logging.info(f"今天是{today.date()}，需要移除的股票有{remove_ids}，將於下一個交易日出售")
            send_msg.append(f"今天是{today.date()}，需要移除的股票有{remove_ids}，將於下一個交易日出售")
            ...
            logging.info(f"今天是{today.date()}，沒有跌停的股票需要出售，不需要調整持倉")
            send_msg.append(f"今天是{today.date()}，沒有跌停的股票需要出售，不需要調整持倉")
    else:
        logging.info(f"今天{today.date()}是休市日")
        send_msg.append(f"今天{today.date()}是休市日")

else:
    logging.info(f"今天{today.date()}是休市")
    send_msg.append(f"今天{today.date()}是休市")
