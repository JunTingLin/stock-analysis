...
from portfolio_management import get_current_portfolio_info
...
force_trading_day = True  # 是否強制今天為交易日，用於debug
...
if today != close.index[-1] and not force_trading_day:
    ...
    if add_ids != set():
        try:
            ...
            _, portfolio_details = get_current_portfolio_info(position_today.position, close, company_basic_info)
        ...
        try:
            ...
            _, portfolio_details = get_current_portfolio_info(position_acc.position, close, company_basic_info)
            trading_info.set_attribute('positions_next', portfolio_details)
