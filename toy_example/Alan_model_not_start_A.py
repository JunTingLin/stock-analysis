...
def build_technical_buy_condition(bias_5_range=(-0.03, 0.10), bias_10_range=(-0.03, 0.10),
    ...
    high_120 = adj_high.rolling(window=new_high_days, min_periods=1).max()
    # 120天收盤新高 = 120天內的最高收盤價(Close)
    close_high_120 = adj_close.rolling(window=new_high_days, min_periods=1).max()
...
buy_signal = (
    ...
    build_technical_buy_condition(
        ...
        bias_120_range=(-0.03, 0.27),  # A版: -3~27%
        bias_240_range=(0.00, 0.31)    # A版: 0~31%
    )['technical_buy_condition'] &
...
def build_sell_condition():
    ...
    # 計算120天收盤新高(Close)
    close_high_120 = adj_close.rolling(window=120, min_periods=1).max()

    # A版賣出條件:
    # 1. 5日線乖離小於-4% 或
    # 2. 10日線乖離小於-4% 或
    # 3. 價格小於「120天收盤新高」的91% 或
    # 4. 60日線乖離大於21% 或
    # 5. 240日線乖離大於32%
    sell_condition = (
        (bias_5 < -0.04) |
